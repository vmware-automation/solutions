...
java weblogic.Deployer -adminurl $url -user weblogic -password $admin_password -deploy -name TestWebApp -appversion VersionA -source $webapps_dir/TestWebApp.war -id TestWebApp01 -targets cluster00,cluster01,cluster02 
echo "SAMPLE APPLICATION DEPLOYED SUCCESSFULLY"

# STARING A DEMO APPLICATION ON THE WEBLOGIC SERVER
java weblogic.Deployer -adminurl $url -user weblogic -password $admin_password -start -name TestWebApp -appversion VersionA -targets cluster00,cluster01,cluster02 
