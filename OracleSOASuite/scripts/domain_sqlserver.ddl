 ...
 * MODIFIED    (MM/DD/YY)
 *    mchinni   05/30/06 - Fix for bug 5163462
 
/**
 * Collaxa Database Schema for SQL Server
 *
 ...
 * For example:
 *
 * osql -Uuser -Ppassword -ddatabase
...
if exists ( select * from sysobjects
            where name = 'admin_list_td' and type = 'V' )
begin
    drop view admin_list_wi
    print "Dropping view admin_list_td"
end
if exists ( select * from sysobjects
...
go
if exists ( select * from sysobjects
            where name = 'test_details' and type = 'U' )
begin
    drop table native_correlation
    print "Dropping table test_details"
end
go
if exists ( select * from sysobjects
...
insert into version values( '2.0.45', 'sqlserver' )
go
...
create nonclustered index ci_custom on cube_instance( test_run_id )
go
...
(
    dockey           varchar( 50 )      constraint xmldoc_pk primary key,
    domain_ref       smallint           not null,
...
(
    conv_id              nvarchar( 128 )             not null,
    conv_type            integer                    not null,
    message_guid         nvarchar( 50 )              not null,
    domain_ref           smallint                   not null,
    ...
    res_process_guid     nvarchar( 50 )              null,
...
(
    cikey           integer                 constraint doc_pk primary key,
    domain_ref      smallint                not null,
...
print "Creating table test_details"
go
...
create clustered index doc_ci_fk on audit_trail( cikey )
go
...
go
print "Creating view admin_list_td"
go
...
as
    ...
    delete from document where domain_ref = @p_domain_ref
    delete from invoke_message where domain_ref = @p_domain_ref
    delete from invoke_message_bin where domain_ref = @p_domain_ref
    delete from dlv_message where domain_ref = @p_domain_ref
    delete from dlv_message_bin where domain_ref = @p_domain_ref
